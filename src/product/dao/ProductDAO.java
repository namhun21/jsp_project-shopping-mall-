...
public class ProductDAO {
	...
	// product 추가하는 method
	public int insertProduct(String pId, String pName, String pContent, String categoryCode, int price, int stock,
			String product_img) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public List<ProductDTO> listAll(int startNo, int endNo) throws SQLException {
		...
		List<ProductDTO> list = null;
		// String sql = "select * from product where pid between ? and ? and isdelete =
		// 0 order by pid desc";
	...
	public ArrayList<ProductDTO> productSelect(String category) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ArrayList<ProductDTO> plist = new ArrayList<ProductDTO>();
		// ,
		conn = DBconnection.getInstance().getConnection();
		String sql = "SELECT pid, pname, categorycode, price, product_img, stock, product_hit FROM product where categorycode = ? and isdelete = 0 ORDER BY product_hit desc";
		try {
			pstmt = conn.prepareStatement(sql);
...
//					pvo.setisDelete(rs.getInt("isdelete"));
				pvo.setPname(rs.getString("pname"));
				pvo.setCategorycode(rs.getString("categorycode"));
				pvo.setPrice(rs.getInt("price"));
				pvo.setProduct_img(rs.getString("product_img"));
				pvo.setStock(rs.getInt("stock"));
				pvo.setProduct_hit(rs.getInt("product_hit"));
				plist.add(pvo);
			}
